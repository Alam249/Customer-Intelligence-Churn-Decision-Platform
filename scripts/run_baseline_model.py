...
def main() -> int:
    ...
    logger.info("Test ROC-AUC=%.4f  PR-AUC=%.4f  F1=%.4f", test_metrics["roc_auc"], test_metrics["pr_auc"], test_metrics["f1"])
    ...
    logger.info("Design matrix condition number: %.1f | sign conflicts: %d/%d features",
                condition_number, len(sign_conflicts), len(feature_names))
    ...
    top_coef["direction"] = np.where(top_coef["coefficient"] > 0, "-> pushes toward CHURN", "-> pushes toward RETENTION")
